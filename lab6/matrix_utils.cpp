...
for (int i = 0; i < rows; i++) {
    ...
    for (int j = 0; j < cols; j++) {
        data[i][j] = matrix[i][j];  // Copy values
    }
}
     */
...
Matrix::Matrix(int** matrix, int n, int m) :  data(matrix), rows(n), cols(m) {
    data = matrix;
    rows = n;
    cols = m;
}
...
// Access element implementations
int* Matrix::operator[](int index) { return data[index]; }
const int* Matrix::operator[](int index) const { return data[index]; }
...
void printMatrix(Matrix data_matrix) {

    int n = data_matrix.getRows();
    int m = data_matrix.getCols();

    int** matrix = data_matrix.getData();
